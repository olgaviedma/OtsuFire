...
#' @param random_seed Integer or NULL. Optional RNG seed; when set, makes
#'   stochastic per-AOI calls reproducible.
#' @param write_aoi_per_folder_shp Logical. If TRUE, writes a per-AOI
#'   shapefile copy into each AOI subfolder for inspection.
#' @param standardize_ba_output Logical. If TRUE, normalises burned-area
#'   per-AOI outputs to a single canonical schema before stitching.
#' @param keep_original_ba Logical. If TRUE, retains the per-AOI native
#'   burned-area output alongside the standardised one.
#' @param stop_on_unused_args Logical. If TRUE, errors when
#'   \code{rescue_args} contains keys not consumed by \code{process_fun};
#'   if FALSE, drops unknown keys silently.
#'
#' @return
#' A list with:
#' \itemize{
...
segmentation_refinement <- function(
  ...
  crop_project_raster_to_aoi <- function(raster_path,
    ...
    if (same_crs && same_res) {
      # F4b: src crop and proj crop would be byte-identical -> write once to proj.
      # No downstream consumer reads out_path_src; skipping the redundant write
      # removes one large raster write per AOI.
      terra::writeRaster(rr, out_path_proj, overwrite = TRUE, gdal = c("COMPRESS=LZW"))
      rr_p <- rr
    } else {
  ...
  clip_ecoregions_to_aoi <- function(ecoregion_shapefile_path,
                                     aoi_geom,
                                     out_path,
                                     ecoregions_sf_cached = NULL,
                                     quiet = TRUE) {
    # F4b: When ecoregions_sf_cached is provided (pre-loaded sf in target CRS,
    # already safe_make_valid_polys'd), skip the per-AOI file read + validation
    # + reproject. Falls back to the original file-read path when NULL.
    if (is.null(ecoregions_sf_cached) &&
        (is.null(ecoregion_shapefile_path) || !file.exists(ecoregion_shapefile_path))) {
      return(NULL)
    }
    ...
    if (!is.null(ecoregions_sf_cached) && inherits(ecoregions_sf_cached, "sf") &&
        nrow(ecoregions_sf_cached) > 0) {
      eco <- ecoregions_sf_cached
      if (sf::st_crs(eco) != sf::st_crs(aoi_u)) {
  ...
  if (!is.null(random_seed)) {
    random_seed <- suppressWarnings(as.integer(random_seed)[1])
    if (!is.na(random_seed)) {
      set.seed(random_seed)
      msg(sprintf("[RNG] segmentation_refinement random_seed=%s", random_seed))
    }
  }
  ...
  # ----------------------------
  # F4b: pre-load the ecoregion layer ONCE (validated, in target CRS) and pass
  # the sf object through the future closure so each worker avoids re-reading
  # and re-reprojecting the full shapefile per AOI. Falls back to per-AOI
  # file read if the pre-load fails.
  # ----------------------------
  eco_preloaded <- NULL
  if (!is.null(ecoregion_shapefile_path) && file.exists(ecoregion_shapefile_path)) {
    eco_preloaded <- tryCatch({
      e0 <- sf::st_read(ecoregion_shapefile_path, quiet = TRUE)
      e0 <- safe_make_valid_polys(e0)
      if (!is.null(e0) && nrow(e0) > 0) {
        if (sf::st_crs(e0) != target_crs_sf) e0 <- sf::st_transform(e0, target_crs_sf)
        e0
      } else {
        NULL
      }
    }, error = function(e) NULL)
    if (isTRUE(verbose) && !is.null(eco_preloaded)) {
      msg(sprintf("[ECO] pre-loaded ecoregion layer once (n=%d) for all AOI workers.",
                  nrow(eco_preloaded)))
    }
  }
      ...
      res <- tryCatch({
        ...
        if (!is.null(ecoregion_shapefile_path) && file.exists(ecoregion_shapefile_path)) {
          ...
          eco_crop_path <- clip_ecoregions_to_aoi(
            ...
            ecoregions_sf_cached = eco_preloaded,
            quiet = TRUE
          )
        }
        ...
        if (!is.null(random_seed) && !is.na(random_seed)) {
          call_args$random_seed <- as.integer(random_seed) + as.integer(i)
        }
