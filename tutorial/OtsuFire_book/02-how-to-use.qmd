...
## Introduction: installing and loading the package

OtsuFire is distributed through the Comprehensive R Archive Network (CRAN)
and can be installed in the same way as any other R package. The recommended
workflow is to install it once from the official source, attach it at the
start of each session, and then call its functions directly.

To install the package, run the following inside an R session connected to
the internet:

```{r}
#| label: install-otsufire
install.packages("OtsuFire")
```

This command pulls the latest released version from CRAN, together with its
mandatory dependencies, and places the package in the default user library.
...
OtsuFire. The `sf` package provides vector data handling, while `terra`
provides raster handling. Both are loaded explicitly at the top of each
worked example:
...
vector operations to external GDAL utilities (`gdal_polygonize`,
`gdalwarp`, `ogr2ogr`) and to a Python interpreter. These tools must be
available on the host system; OtsuFire is then told where to find them
through configuration paths set later in this tutorial.
...
composed of three operational steps: building a configuration object that
bundles inputs and parameters, detecting candidate patches through
stratified Otsu thresholding and seed-and-grow segmentation, and scoring
those candidates through rule-based filtering into the canonical
keep / review / drop decision layer.
...
hotspot detections, which are only available from 1995 onwards. The tutorial
therefore separates two cases: years with active hotspot support, where the
full rule-based system can be applied, and earlier years, for which the
workflow records the hotspot filter as not applied rather than as failed.
The first case is illustrated below; earlier years will be covered in a
later subsection.

### Year with active hotspot support

This worked example runs the unsupervised stage for a target year in which
active-fire hotspot detections are available. The example uses **2025** as
the target year and the **balanced** detection preset, but the same script
applies to any year from 1995 onwards by changing the two user settings at
the top of the user-settings block.
...
Some paths depend on `target_year` (`change_index`, `reference_burned_map`,
`previous_year_burned`, `hotspots`). Others depend on `corine_year`
(`vegetation_map`, `burnable_mask`).
...
starts. Optional inputs (`previous_year_burned` may be NULL for 1985,
`hotspots` is NULL for pre-hotspot years) are excluded from the failure
condition.
...
- `seed_threshold` — minimum change value needed to create a burned seed
  pixel. Higher values mean fewer seeds and more conservative detection.
- `growth_delta` — amount by which the grow threshold is relaxed relative
  to the seed threshold. Higher values mean easier patch expansion.
- `minimum_growth_threshold` — absolute floor below which region growing
  is not allowed to go. Higher values mean tighter, more conservative
  patch growth.
- `minimum_seed_pixels` — minimum number of seed pixels needed to keep a
  candidate component. Higher values remove small isolated detections.
- `*_by_vegetation` — per-class overrides for the global values above,
  used when one global value is too coarse and vegetation types need
  different behaviour.
...
These are the current package defaults written explicitly so the user can
see them and modify them if needed.

- `aoi_buffer_m` — buffer used around AOI boundaries during refinement to
  reduce edge artefacts.
- `omission_buffer_m` — reserved cleanup buffer for omission-side
  adjustments. Zero means no extra omission buffer.
- `minimum_detected_area_m2` — minimum polygon area kept after refinement.
  Very permissive by default.
- `merge_overlaps` — if TRUE, overlapping refined polygons are merged.
- `merge_overlaps_buffer_m` — optional buffer before overlap merging. Zero
  means merge only direct overlaps.
...
- `support_buffer_m` — buffer used when checking local contextual support
  around candidate patches.
- `previous_fire_exclusion_buffer_m` — buffer around previous-year burns
  used to detect temporal conflicts.
- `previous_fire_cleanup_buffer_m` — cleanup buffer used after overlap
  removal with previous-year burns.
- `minimum_remaining_area_m2` — minimum area a patch must retain after
  temporal cleanup.
- `reference_buffer_m` — buffer used when comparing against
...
`output_dir` must be the root results folder only. The package itself
appends `<target_year>/DETERMINISTIC/<run_name>`, so neither `target_year`
nor `DETERMINISTIC` should be added manually.
...
#### Step 1. Build the unsupervised configuration

At this stage the workflow is only being prepared. Nothing is detected or
scored yet. This is the right moment to inspect the selected preset, the
parameter groups and the input paths.
...
seed pixels are created, how far candidate patches can grow, and how much
behaviour differs by vegetation / land-cover class.
...
This is the native deterministic scoring behaviour. Setting `keep_pool = NULL`
tells the package to use its built-in same-year local fallback.
...
external reference burned-area dataset using `validate_fire_maps()`.

```{r}
#| label: validation-keep-only
...
::: {.callout-important}
## Observability is temporal, not a pixel mask
`observability_raster` triggers a **temporal, whole-fire** observability
filter. For each reference fire the function compares the fire DOY
(`ref_end_doy_col`, fallback `ref_start_doy_col`) against the maximum
observable DOY inside the polygon. A fire with no valid observable DOY, or
whose date is after the last observation, is dropped from the reference
*before* rasterization — it contributes 0 TP / 0 FN / 0 evaluated area and
is absent from omission, recall, F1, IoU and per-fire detection. It is
**not** a pixel-level cloud/valid-data mask: partially observed fires are
kept whole, and no per-pixel denominator is introduced. Residual per-pixel
non-observability inside kept fires is reported only for audit, in
`val$observability_audit` and the `02_OBSERVABILITY/OBSERVABILITY_*` CSVs,
and never changes any metric. The reference cache is content-aware and
rebuilds automatically when the observability raster, burnable mask,
resolution, CRS, grid, study-area mask, reference content or the
observability rule changes; `force_reprocess_ref = TRUE` forces a rebuild.
:::
