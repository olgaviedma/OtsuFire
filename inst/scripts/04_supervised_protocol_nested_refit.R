# =============================================================================
# CANONICAL versioned supervised usage script  (04 / 6)
# OtsuFire — training_protocol = "nested_refit" example
# =============================================================================
# *** CANONICAL, VERSIONED COPY (inst/scripts/). External 00_USAGE copies are
#     WORKING COPIES. *** Targets OtsuFire >= 0.5.0.
#
# FOCUS: the nested_refit protocol, selected ONCE in the builder via
#   training_protocol = "nested_refit" (+ oof_sampling = "capped"). This is the
#   candidate protocol (nested early-stopping + refit, fold-specific imputation).
#   It differs from 03_supervised_protocol_legacy.R ONLY on the protocol axis,
#   so the two scripts form a controlled legacy-vs-nested pair.
#
...
TRAINING_PROTOCOL <- "nested_refit"   # <<< the axis this example pins
OOF_SAMPLING      <- "capped"         # "capped" | "full"
...
main <- function() {
  ...
  cfg <- build_supervised_burned_config(
    ...
    cap_otsu           = caps$otsu,
    training_protocol  = TRAINING_PROTOCOL,   # "nested_refit"
  ...
  stopifnot(
    identical(cfg$train_control$training_protocol, "nested_refit"),
    identical(cfg$train_control$oof_sampling, "capped")
  )
...
if (isTRUE(RUN)) main() else
  message("04_supervised_protocol_nested_refit.R sourced with RUN=FALSE (no compute).")
# =============================================================================
# End of nested_refit-protocol example.
# =============================================================================
