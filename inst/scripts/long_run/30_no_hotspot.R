...
cfg_B <- .mk_long_cfg(feature_whitelist_override = NOHS_BASE_38, out_base = NOHS_ENGINE)
saveRDS(list(B = cfg_B), file.path(PROF, "cfgs_no_hotspot.rds"))
...
md <- c("# LONG BALANCED — NO-HOTSPOT profile (spectral-historical ablation)",
  ...
  sprintf("- Caps (two negative sources): random %.2f / otsu %.2f.",
          CAP_RANDOM, CAP_OTSU),
