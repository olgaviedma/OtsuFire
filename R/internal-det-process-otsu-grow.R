...
#' @param use_original Logical. Reserved/legacy flag; not used by this workflow.
#'   Kept for API compatibility with \code{process_otsu_rasters_()}.
#' @param output_corine_vector_dir Character or NULL. Output directory for the
#'   reclassified CORINE vector layer when \code{vectorize=TRUE} (unused by
#'   this workflow but preserved for API compatibility).
#' @param random_seed Integer or NULL. Optional RNG seed used to make
#'   sample-based Otsu estimation reproducible.
#'
#' @return
...
process_otsu_rasters_grow <- function(
  ...
  if (!is.null(random_seed)) {
    ...
    if (!is.na(random_seed)) {
      ...
      message(sprintf("[RNG] process_otsu_rasters_grow random_seed=%s", random_seed))
    }
  }
  ...
  # Otsu smooth histogram package. Block 9b: OtsuSeg moved from Imports
  # to Suggests; resolved at call time via getExportedValue().
  if (!requireNamespace("OtsuSeg", quietly = TRUE)) {
    stop("Package 'OtsuSeg' is required for the Otsu-smoothed-histogram ",
         "step in process_otsu_rasters_grow(). Install it with ",
  ...
  # F4b: lazy one-time materialization of r and unit_r values for the fallback
  # extraction branch. Previously each fallback unit called
  # terra::ifel(unit_r == uid, r, NA) + terra::values(r_u), which scans the
  # full raster per unit. We materialize once (only if any fallback happens)
  # and subset by UNIT_ID in memory. Freed after the loop.
  fb_r_vals <- NULL
  fb_u_vals <- NULL
  ...
  for (k in seq_len(nrow(unit_table))) {
    ...
    if (is.null(vals_u) || length(vals_u) < min_unit_sample) {
      if (is.null(fb_r_vals)) {
        # First fallback hit: materialize both rasters once as flat vectors.
        # Replicates the per-unit terra::ifel(unit_r == uid, r, NA) + terra::values
        # pattern with ONE full raster read total instead of one per fallback unit.
        # Important: terra::ifel(NA_cond, yes, no) returns `yes` (the r branch) on
        # NA cells of the condition. To preserve byte-for-byte equivalence with
        # the prior code path, the mask treats NA in unit_r as matching too,
        # i.e. mask <- is.na(unit_r) | unit_r == uid.
        fb_r_vals <- terra::values(r, mat = FALSE)
        fb_u_vals <- terra::values(unit_r, mat = FALSE)
      }
      unit_mask <- is.na(fb_u_vals) | fb_u_vals == uid
      vals_all <- fb_r_vals[unit_mask]
      vals_all <- vals_all[is.finite(vals_all)]
      if (length(vals_all) > max_unit_sample) vals_all <- sample(vals_all, size = max_unit_sample)
      vals_u <- vals_all
      rm(vals_all, unit_mask)
      unit_table$N_SAMPLE[k] <- length(vals_u)
    }
  ...
  # F4b: release full-raster fallback vectors (if they were materialized).
  if (!is.null(fb_r_vals) || !is.null(fb_u_vals)) {
    rm(fb_r_vals, fb_u_vals); gc(FALSE)
  }
