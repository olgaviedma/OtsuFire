...
```
   ...
   Config A legacy                    feature_whitelist_override = 38 non-hotspot
   Config B nested_refit (DEPLOYED)   use_hotspots = FALSE
   Config C nested_refit-full diag    nested_refit (B) + OOF-full diagnostic
   score + map + EFFIS                score + map + EFFIS
```
...
| `20_full.R` | FULL profile: Config A/B/C, deploy B, score + map + EFFIS. |
