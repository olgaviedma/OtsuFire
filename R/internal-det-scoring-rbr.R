#' Phase 2 scoring: RBR-based keep-like flagging
#'
...
#' probabilities), and (iv) assigns a binary \code{keep_like} decision.
#'
...
#' @param random_seed Integer or NULL. Seed used when subsampling the keep
#'   pool to \code{max_keep_samples}.
#' @param normalize_corine_field Logical. If TRUE, normalises the CORINE
#'   NA-fraction column name on the output to \code{corine_na_col}.
#' @param corine_na_col Character. Canonical CORINE NA-fraction column name.
#' @param corine_alt_cols Character vector. Alternative column names that
#'   should be renamed to \code{corine_na_col} when present.
#' @param fill_na_corine Logical. If TRUE, fills missing CORINE NA-fraction
#'   values with \code{fill_na_corine_value}.
#' @param fill_na_corine_value Numeric. Replacement value used when
#'   \code{fill_na_corine = TRUE}.
#'
#' @return A list with:
#' \itemize{
...
score_rbr_keep_classes <- function(
  ...
  if (!is.null(random_seed)) {
    ...
    if (!is.na(random_seed)) {
      ...
      msg(sprintf("[RNG] score_rbr_keep_classes random_seed=%s", random_seed))
    }
  }
  ...
  if (is.null(keep_pool)) {
    ...
    # Single-pass extraction: collect keep_vals (for quantiles) AND keep_medians
    # (halves terra::extract() calls vs. the previous two-loop design; identical outputs)
    keep_vals    <- numeric(0)
    ...
    # F4: release keep-pool intermediates before the main scoring loop (keep_pool already
    # holds what it needs). keep_vals can be up to max_keep_samples doubles (~40 MB at 5e6).
    rm(keep_polys, keep_vals, keep_medians); gc(FALSE)
