test_that("process_otsu_rasters runs on synthetic raster and outputs valid shapefile", {
  ...
  raster_path <- file.path(tmp_dir, "raster_synthetic_mosaic.tif")
  ...
  # Comprobar que tiene geometria valida y al menos 1 columna
  expect_s3_class(shp, "sf")
  expect_true(nrow(shp) > 0)
  expect_true("geometry" %in% names(shp))

  # Si esperas campos especificos, puedes incluirlos opcionalmente:
  expected <- c("DN")  # Solo si sabes que se debe incluir
  expect_true(all(expected %in% names(shp)), info = paste("Faltan campos:", paste(setdiff(expected, names(shp)), collapse = ", ")))
})
